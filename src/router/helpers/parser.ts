 ...
 * @param config.allowedTypes - allowed file types (default png, jpeg, jpg, gif)
 */
function parseFile(field: string, config?: { multiple?: boolean, maxCount?: number, fileSize?: number, allowedTypes?: string[]}): RequestHandler {

	const multerInstance = getMulter({ maxFileSize: config?.fileSize, allowedTypes: config?.allowedTypes });

	if(config?.multiple || (config?.maxCount && config?.maxCount > 1)){
